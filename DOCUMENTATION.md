...
- **Refonte UI « Tailwind UI — Application UI »** (2026) : composants CSS réutilisables inspirés des blocs Tailwind UI (Product / Application UI) aux couleurs de la charte — `page-heading` (titre + sous-titre + actions), `stat-card` (icône teintée + grande valeur), `badge` (statut vert/orange/rouge/or), tables modernes (`table-wrap`/`table-head`/`table-body`), `empty-state`, `alert` (succès/danger/info/avertissement), `btn-secondary` et `input-field`. Appliqués aux dashboards admin & superviseur, liste des employés, lieux de travail, suivi des présences, rendements, formulaires et page d'accueil.
- **Vérification DNS Resend** : script `scripts/check-resend-dns.sh <domaine>` qui vérifie les enregistrements MX/SPF/DKIM/DMARC (`send.` + `resend._domainkey.` + `_dmarc.`) attendus par Resend — utile avant la mise en production des emails.
