...
class MissingArrivalNotification extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        if ($notifiable->role === 'administrateur') {
            $subject = 'Notification d\'absence d\'arrivée - Administrateur';
            $greeting = 'Bonjour Administrateur,';
        } elseif ($notifiable->role === 'Superviseur') {
            $subject = 'Notification d\'absence d\'arrivée d\'un membre de votre équipe';
            $greeting = 'Bonjour Superviseur,';
        } else {
            $subject = 'Notification d\'absence d\'arrivée';
            $greeting = 'Bonjour,';
        }
        ...
        return (new MailMessage)
                    ->subject($subject)
                    ->greeting($greeting)
                    ->line('Un employé n\'a pas marqué son arrivée aujourd\'hui avant 10h.')
    ...
    public function toArray(object $notifiable): array
    {
        ...
        return [
            ...
            'date' => $this->date,
            'message' => $messagePrefix . 'L\'employé ' . $this->employe->nom . ' n\'a pas marqué son arrivée avant 10h le ' . $this->date,
