...
<a href="{{ route('secretaire.services') }}" class="block rounded-md bg-pharaoh-gold px-3 py-2 text-base font-medium text-white">Services</a>
...
<div class="max-w-4xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
    ...
    <div class="pharaoh-card overflow-hidden">
        ...
        <div class="table-scroll"><table class="min-w-full"><thead class="table-head"><tr><th>Heure</th><th>Type</th><th>Détails</th><th class="text-right">Montant</th><th></th></tr></thead>
            <tbody class="table-body">@forelse($services as $svc)<tr><td class="px-6 py-4 text-sm text-gray-500">{{ $svc->created_at->format('H:i') }}</td><td class="px-6 py-4 text-sm font-medium text-[#080808]">{{ $typesPhoto[$svc->type] ?? $svc->type }}</td><td class="px-6 py-4 text-sm text-gray-500">{{ $svc->details ?? '—' }}</td><td class="px-6 py-4 text-sm font-semibold text-right text-blue-700">{{ number_format($svc->montant, 0, ',', '.') }} FCFA</td>                            <td class="px-6 py-4 text-right space-x-2"><a href="{{ route('secretaire.services.edit', $svc->id) }}" class="text-pharaoh-gold hover:text-pharaoh-bronze text-sm font-medium">Modifier</a><form action="{{ route('secretaire.services.destroy', $svc->id) }}" method="POST" class="inline" onsubmit="return confirm('Supprimer ?')">@csrf @method('DELETE')<button type="submit" class="text-red-500 hover:text-red-700 text-sm">Supprimer</button></form></td></tr>@empty<tr><td colspan="5" class="px-6 py-12 text-center text-sm text-gray-400">Aucun service aujourd'hui</td></tr>@endforelse</tbody>
        </table></div>
    </div>
</div>
@endsection
