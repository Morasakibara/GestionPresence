...
@section('header')
<div class="flex items-center justify-between"><span>Retraits — Secrétaire</span></div>
@endsection
...
<a href="{{ route('secretaire.retraits') }}" class="rounded-md bg-pharaoh-gold px-3 py-2 text-sm font-medium text-white" aria-current="page">Retraits</a>
...
<a href="{{ route('secretaire.retraits') }}" class="block rounded-md bg-pharaoh-gold px-3 py-2 text-base font-medium text-white">Retraits</a>
...
<div class="max-w-4xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
    ...
    <div class="pharaoh-card p-6 mb-6 border-pharaoh-gold/30 bg-gradient-to-br from-[#FBF3E6] to-white">
        <div class="text-xs font-medium uppercase tracking-wide text-gray-500">Somme disponible</div>
    ...
    <div class="pharaoh-card p-6 mb-6">
        <form action="{{ route('secretaire.retraits.store') }}" method="POST" class="space-y-4">
            @csrf
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div><label class="input-label">Motif</label><input type="text" name="motif" class="input-field mt-1" value="{{ old('motif') }}" placeholder="Motif du retrait..." required></div>
            </div>
            <button type="submit" class="btn-gold bg-red-600 hover:bg-red-700 focus:ring-red-500"><svg class="-ml-1 mr-1.5 h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"/></svg> Enregistrer</button>
        </form>
    </div>
    ...
    <div class="pharaoh-card overflow-hidden">
        ...
        <div class="table-scroll"><table class="min-w-full"><thead class="table-head"><tr><th>Heure</th><th>Motif</th><th class="text-right">Montant</th></tr></thead>
            <tbody class="table-body">@forelse($retraits as $r)<tr><td class="px-6 py-4 text-sm text-gray-500">{{ $r->created_at->format('H:i') }}</td><td class="px-6 py-4 text-sm text-[#080808]">{{ $r->motif }}</td><td class="px-6 py-4 text-sm font-semibold text-right text-red-600">{{ number_format($r->montant, 0, ',', '.') }} FCFA</td></tr>@empty<tr><td colspan="3" class="px-6 py-12 text-center text-sm text-gray-400">Aucun retrait aujourd'hui</td></tr>@endforelse</tbody>
        </table></div>
    </div>
</div>
@endsection
