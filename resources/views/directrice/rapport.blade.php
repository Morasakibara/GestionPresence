...
@section('header')
<div class="flex items-center justify-between"><span>Rapport — Directrice</span></div>
@endsection
...
<a href="{{ route('directrice.rapport') }}" class="rounded-md bg-pharaoh-gold px-3 py-2 text-sm font-medium text-white" aria-current="page">Rapport</a>
@endsection
...
<a href="{{ route('directrice.rapport') }}" class="block rounded-md bg-pharaoh-gold px-3 py-2 text-base font-medium text-white">Rapport</a>
@endsection

@section('content')
<div class="max-w-5xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
    <h1 class="page-heading-title mb-6">Rapport financier</h1>

    <!-- Filtres -->
    <div class="pharaoh-card p-6 mb-6">
        <form method="GET" action="{{ route('directrice.rapport') }}" class="flex flex-wrap items-end gap-4">
            <div>
                <label class="input-label">Date début</label>
                <input type="date" name="date_debut" class="input-field mt-1" value="{{ $dateDebut }}">
            </div>
            <div>
                <label class="input-label">Date fin</label>
                <input type="date" name="date_fin" class="input-field mt-1" value="{{ $dateFin }}">
            </div>
            <button type="submit" class="btn-gold">Filtrer</button>
            <a href="{{ route('directrice.rapport.export', ['date_debut' => $dateDebut, 'date_fin' => $dateFin]) }}" class="btn-secondary">
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-4 mb-6">
        <div class="stat-card">
            <div class="stat-card-value text-green-600">{{ number_format($totalCommandes, 0, ',', '.') }}</div>
            <div class="stat-card-label">Commandes (FCFA)</div>
        </div>
        <div class="stat-card">
            <div class="stat-card-value text-blue-600">{{ number_format($totalServices, 0, ',', '.') }}</div>
            <div class="stat-card-label">Services (FCFA)</div>
        </div>
        <div class="stat-card">
            <div class="stat-card-value text-red-600">{{ number_format($totalRetraits, 0, ',', '.') }}</div>
            <div class="stat-card-label">Retraits (FCFA)</div>
        </div>
        <div class="stat-card border-pharaoh-gold/30 bg-[#FBF3E6]">
            <div class="stat-card-value text-pharaoh-gold">{{ number_format($totalCommandes + $totalServices - $totalRetraits, 0, ',', '.') }}</div>
            <div class="stat-card-label font-semibold">Bilan net (FCFA)</div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div class="pharaoh-card p-6">
            ...
            @forelse($commandesParType as $type => $montant)
                <div class="flex items-center justify-between py-2 {{ !$loop->last ? 'border-b border-gray-100' : '' }}">
                    <span class="text-sm text-gray-700">{{ $typesCaisse[$type] ?? $type }}</span>
                    <span class="text-sm font-semibold text-green-700">{{ number_format($montant, 0, ',', '.') }} FCFA</span>
                </div>
            @empty
        ...
        <div class="pharaoh-card p-6">
            ...
            @forelse($servicesParType as $type => $montant)
                <div class="flex items-center justify-between py-2 {{ !$loop->last ? 'border-b border-gray-100' : '' }}">
                    <span class="text-sm text-gray-700">{{ $typesCaisse[$type] ?? $type }}</span>
                    <span class="text-sm font-semibold text-blue-700">{{ number_format($montant, 0, ',', '.') }} FCFA</span>
                </div>
            @empty
                <p class="text-sm text-gray-400">Aucune donnée</p>
            @endforelse
